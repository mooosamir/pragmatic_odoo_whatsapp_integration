...
class SliderBuilder(WebsiteSale):
    ...
    def get_product_listing(self, name=False, **kwargs):
        ...
        if name:
            ...
            error = True if not (products or filters) else False
        if error:
    ...
    def load_more_category_brand(self, **kw):
        ...
        item_ids = [int(i) for i in item_ids.split(',')] if \
            item_ids and type(item_ids) == str else [int(i) for i in item_ids] if item_ids else False
        response = False
    ...
    def get_category_brand(self, name, item_ids=False):
        ...
        if name == 'category-slider':
            ...
            items_count = pub_categ_obj.sudo().search_count(
                [('website_id', 'in', [False, request.website.id]), ('image_1920', '!=', False)])
        else:
            domain = [('website_id', 'in', [False, request.website.id]), ('logo', '!=', False),
                      ('id', 'in', item_ids), ('website_published', '=', True)] if item_ids \
                else [('website_published', '=', True), ('website_id', 'in', [False, request.website.id]),
                      ('logo', '!=', False)]
            items = brand_obj.sudo().search(domain, order='id desc')
            items_count = brand_obj.sudo().search_count(
    ...
    def get_brand_category_template(self, **kw):
        ...
        item_ids = kw.get('item_ids', False)
        item_ids = [int(i) for i in item_ids.split(',')] if \
            item_ids and type(item_ids) == str else [int(i) for i in item_ids] \
    ...
    def category_brand_render(self, **kwargs):
        ...
        item_ids = [int(i) for i in item_ids.split(',')] if item_ids and type(item_ids) == str \
            else [int(i) for i in item_ids] if item_ids else False
    ...
    def best_seller_products(self, limit=10):
        ...
        select * from sale_report where website_id=%s AND 
        state in ('sale','done') AND date BETWEEN %s and %s
